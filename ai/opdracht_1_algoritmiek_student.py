...
def my_sort(lst) -> list:
    ...
    while i < len(list1) - 1:
        if list1[i] > list1[i+1]:
            # print(f"voor {list1}, i {i}")
            # buffer voor overschreven waarde
            buf = list1[i]
            list1[i] = list1[i+1]
            list1[i+1] = buf
            # list1[i] = list1[i+1]
            # list1[i+1] = list1[i]
            i = 0
            # print(f"na {list1}, i {i}")
        # else ga door
        else:
            # print(f"{i} i++ is niet groter, {list1}")
            i += 1
...
"""
        ...
        -   Stel je hebt een lijst met de waarden 1 tot en met 4.
            ...
            Hoeveel vergelijkingen zijn er nodig?
"""
# best: [1, 2, 3, 4] 3 vergelijkingnen
# worst: [4, 3, 2, 1] 12 vergelijkingen
"""
...
def linear_search_recursive(lst: list, target):
    """
